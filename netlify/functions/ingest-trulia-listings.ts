...
const TRULIA_BASE_URL = 'https://www.trulia.com';
...
function normalizeIncomingListing(input: unknown): NormalizedListingInput | null {
  ...
  const listingUrl = toAbsoluteUrl(typeof candidate.listingUrl === 'string' ? candidate.listingUrl : undefined, TRULIA_BASE_URL);
  if (!listingUrl) {
    return null;
  }
...
export const handler: Handler = async (event) => {
  ...
  const configuredToken = process.env['TRULIA_INGEST_TOKEN'];
  if (!configuredToken) {
    return {
      statusCode: 500,
      headers: defaultHeaders,
      body: JSON.stringify({ error: 'Missing TRULIA_INGEST_TOKEN environment variable.' })
    };
  }
