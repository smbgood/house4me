...
function normalizeIncomingListing(input: unknown): NormalizedListingInput | null {
  ...
  return {
    ...
    listingUrl,
    imageUrl: toAbsoluteUrl(typeof candidate.imageUrl === 'string' ? candidate.imageUrl : undefined, ZILLOW_BASE_URL),
...
export const handler: Handler = async (event) => {
  ...
  const configuredToken = process.env['ZILLOW_INGEST_TOKEN'];
  if (!configuredToken) {
    return {
      statusCode: 500,
      headers: defaultHeaders,
      body: JSON.stringify({ error: 'Missing ZILLOW_INGEST_TOKEN environment variable.' })
    };
  }
